...
const argv = yargs
  ...
  .option('depth', {
    alias: 'd',
    default: 1,
    description: 'Crawler URL recursion depth',
    type: 'number',
  })
  .option('breadth', {
    alias: 'b',
    default: 3,
    description: 'Crawler URL recursion breadth / branching factor',
    type: 'number',
  })
  .option('seed-list', {
    alias: 'i',
    description: 'Crawler seed list',
    type: 'string',
  })
  .option('start', {
    alias: 's',
    default: 0,
    description: 'Seed list start index',
    type: 'number',
  })
  .option('end', {
    alias: 'e',
    default: -1,
    description: 'Seed list end index (exclusive)',
    type: 'number',
  })
  .option('port', {
    alias: 'p',
...
let queue_remaining = 0
let completed_success = 0
let completed_error = 0
...
;(async () => {
  if (MANUAL_MODE) {
    ...
    console.log(
      `${timestamp()}Workers: ${WORKERS}; Recursion: d=${MAX_RECURSION_DEPTH}, b=${MAX_RECURSION_BREADTH}.`
    )
    ...
    cluster.task(async ({ page, data }) => {
      if (argv.verbose)
        console.log(
          `${timestamp()}puppeteer@${argv.port}: Visiting ${
            data.url
      ...
      await page.exposeFunction('wasmModuleCallback', (buf, loc, dom) => {
        ...
        const fname =
          dom.split('.').join('_') + '_' + buf.substring(0, 16) + '.wasm.b64'
        const date = new Date()
        const meta = ''
        meta += 'file: ' + fname + '\n'
        meta += 'url: ' + loc + '\n'
        ...
        if (argv.verbose)
          console.log(`${timestamp()}Detected .wasm module via hook.`)
      })
      ...
      if (recursion < MAX_RECURSION_DEPTH) {
        ...
        for (
          let i = 0;
          elems && elems.length > 0 && i < MAX_RECURSION_BREADTH;
          i++
        ) {
    ...
    const urls = (await readFileAsync(argv['seed-list'])).toString().split('\n')
    let i = 0
    for (let url of urls) {
    ...
    if (argv.verbose)
      console.log(
        `${timestamp()}puppeteer@${argv.port}: Done. Time: ${
          (now() - start) / 1000
        }s. Rem.: ${queue_remaining}. OK.: ${completed_success}. Err.: ${completed_error}`
      )
  }
})()
